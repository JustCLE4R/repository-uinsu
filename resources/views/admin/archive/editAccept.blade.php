                        ...
                        <form method="post" action="/admin/archive/{{ $archive->id }}/accept" enctype="multipart/form-data">
                          ...
                          <div class="row justify-content-between page page-active" >
                            <div class="col-12">
                              ...
                              <div class="col-lg-4 col-md-6 col-sm-12 my-2">
                                  ...
                                  <input type="date" class="form-control {{ $errors->has('date') ? 'is-invalid' : '' }}"  value="{{ old('date') ?? date('Y-m-d', strtotime($archive->date)) }}" id="date" name="date" >
                                  @error('date')
                                    <div style="color: red; ">{{ $message }}</div>
                                  @enderror      
                              </div>
                                ...
                                <div class="col-lg-4 col-md-6 col-sm-12 my-2">
                                  ...
                                  <select class="form-control {{ $errors->has('subject_id') ? 'is-invalid' : '' }}"  id="subject_id" name="subject_id" >
                                    ...
                                    @foreach ($subjects as $subject)
                                        <option value="{{ $subject->id }}" {{ old('subject_id') == $subject->id || $archive->subject_id == $subject->id ? 'selected' : '' }}>{{ '[' . $subject->code . '] ' . $subject->name }}</option>
                                    @endforeach
                                  </select>
                                  @error('subject_id')
                                    <div style="color: red; ">{{ $message }}</div>
                          ...
                          <div class="col-lg-4 col-md-6 col-sm-12 my-2">
                            ...
                            <select class="form-control {{ $errors->has('visibility') ? 'is-invalid' : '' }}"  id="visibility" name="visibility">
                              ...
                              <option value="public" {{ old('visibility') == 'public' || $archive->visibility == 'public' ? 'selected' : '' }}>Public</option>
                              <option value="private" {{ old('visibility') == 'private' || $archive->visibility == 'private' ? 'selected' : '' }}>Private</option>
                            </select>
                            @error('visibility')
                              <div style="color: red; ">{{ $message }}</div>
                            @enderror      
